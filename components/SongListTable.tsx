...
const SongListTable: React.FC<SongListTableProps> = ({
    ...
    return (
        <div className="animate-fade-in">
            <div className="grid grid-cols-[auto_1fr_1fr_auto_auto] gap-4 px-4 py-2 border-b border-white/[0.08] text-xs font-bold uppercase tracking-wider text-zinc-500 mb-2 items-center">
            ...
            <div className="space-y-1">
                {songs.map((song, index) => {
                    ...
                    return (
                        <div
                            key={song.id}
                            className={`group grid grid-cols-[auto_1fr_1fr_auto_auto] gap-4 px-4 py-3 rounded-lg items-center hover:bg-white/[0.04] transition-colors ${isCur ? 'bg-white/[0.08]' : ''}`}
                        >
                            {/* Index / Play Button */}
                            <div className="w-8 text-center text-sm font-medium text-zinc-500 relative flex justify-center items-center">
                            ...
                            <div className="flex items-center gap-3 min-w-0">
                                ...
                                <div className="min-w-0 flex-1">
                                    <div className={`font-bold truncate text-sm ${isCur ? 'text-brand' : 'text-white'}`}>{song.title}</div>
                                    <div className="text-zinc-500 text-xs truncate group-hover:text-white transition-colors cursor-pointer hover:underline">{song.artist}</div>
                                </div>
                            </div>
                            ...
                            {/* Date Added */}
                            <div className="hidden lg:flex justify-end text-sm text-zinc-500">
                                {formatDate(song.dateAdded)}
                            </div>

                            {/* Duration & Heart */}
                            <div className="w-16 flex items-center justify-end gap-3 text-sm text-zinc-500 font-tabular-nums">
                                <button
